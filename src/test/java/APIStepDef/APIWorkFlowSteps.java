...
//-----------------------------------------------------------------------------------------
    ...
    @Then("the status code for retrieved employee is {int}")
    public void theStatusCodeForRetrievedEmployeeIs(int statusCode) {
        response.then().assertThat().statusCode(statusCode);
    }
    ...
    public void aRequestIsPreparedForPartialEmployeeUpdate() {

        request = given().header(APIConstants.HEADER_CONTENT_TYPE_KEY,
                        APIConstants.HEADER_CONTENT_TYPE_VALUE).
                header(APIConstants.HEADER_AUTHORIZATION_KEY, GenerateTokenStep.token).
                body(APIPayloadConstants.partialEmployeeUpdatePayload());
    }

    @When("a PATCH call is made to partial employee update")
    public void aPATCHCallIsMadeToPartialEmployeeUpdate() {

        response = request.when().patch(APIConstants.PARTIAL_UPDATE_URI);
        response.prettyPrint();
    }

    @Then("the status code for partial employee update is {int}")
    public void theStatusCodeForPartialEmployeeUpdateIs(int statusCode) {

        response.then().assertThat().statusCode(statusCode);
    }

    @And("the employee update contains key {string} and value {string}")
    public void theEmployeeUpdateContainsKeyAndValue(String key, String value) {
    ...
    public void aRequestIsPreparedForRetrievingThePartiallyUpdatedEmployee() {
        request = given().header(APIConstants.HEADER_CONTENT_TYPE_KEY,APIConstants.HEADER_CONTENT_TYPE_VALUE).
                header(APIConstants.HEADER_AUTHORIZATION_KEY,
                        GenerateTokenStep.token)
                .queryParam("employee_id", employee_id);
    }
    ...
    @Then("the status code for partially updated employee is {int}")
    public void theStatusCodeForPartiallyUpdatedEmployeeIs(int statusCode) {

        response.then().assertThat().statusCode(statusCode);
    }

    @And("this updated employee data at {string} object matches with the data used to update the employee")
    public void thisUpdatedEmployeeDataAtObjectMatchesWithTheDataUsedToUpdateTheEmployee
            (String employeeObject, io.cucumber.datatable.DataTable dataTable) {
        // Write code here that turns the phrase above into concrete actions
        List<Map<String, String>> expectedData = dataTable.asMaps();
        //since we need whole object, we are calling .get method instead of .getString method
        ...
        for (Map<String, String> map2 : expectedData
        ) {
            //to keep the order and to avoid duplicates
